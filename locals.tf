locals {
  ...
  windows_nodes = (length([ for v in local.node_poolls : v if lower(v.os_type) == "windows" ]) > 0 ? true : false)

  valid_windows_config = (local.windows_nodes && var.windows_profile == null ?
