...
| configure\_sp\_subnet\_role | Add Network Contributor role for service principal on input subnets. | `bool` | `true` | no |
...
| node\_pool\_subnets | default node pool vnet subnet info | <pre>map(object({<br>                  id                  = string<br>                  resource_group_name = string<br>                  security_group_name = string<br>                }))</pre> | `{}` | no |
